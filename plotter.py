...
for i in range(len(acc)):
    if (i == 0):
        pos.append([acc[i][0]*i*dt*dt*0.5, acc[i][1]*i*dt*dt*0.5, acc[i][2]*i*dt*dt*0.5])
    else:
        pos.append([pos[i-1][0] + acc[i][0]*dt*i*dt*0.5, pos[i-1][1] + acc[i][1]*dt*i*dt*0.5, pos[i-1][2] + acc[i][2]*i*dt*dt*0.5])
pos = np.array(pos)
